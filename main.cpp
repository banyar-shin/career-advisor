...
{
    std::string choice = "y";
    while(choice == "y" || choice == "Y") //for repeat program prompt
    {
        ...
        std::unordered_map<std::string, std::vector<std::string> > ourmap = jobMap ("Jobs.txt"); //Map linking job categories with jobs
        ...
        survey(); //Optional survey for taking feedback

        std::cout << "Do you want to start over from the beginning? ('y' to repeat): "; //Repeat program prompt
        std::cin >> choice; //Ask for y/n input
    }
